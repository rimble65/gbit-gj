...
public class GraphicUnLockManager : MonoBehaviour
{
    ...
    void AddSelectPoint(GameObject obj)
    {
        if (IsPressed() && null != obj)
        {
            ...
            if (null != rtTrans && !_lstSelectPoints.Contains(rtTrans))
            {
                ...
                if (_lstPassword.Count == _lstPoints.Count)
                {
                    if (CheckAnswer() == true)
                    {
                        path.SetActive(false);
                        answerPic.SetActive(true);
                        Image img=answerPic.GetComponent<Image>();
                        img.DOColor(new Color(img.color.r, img.color.g, img.color.b, 1), 2f);
                    }
                }
            }
        }
    ...
    private bool CheckAnswer()
    {
        ...
        if (guanka == 0)
        {
            for (int i = 0; i < _lstPassword.Count - 1; i++)
            {
                int num = _lstPassword[i];
                switch (num)
                {
                    case 0:
                        if (_lstPassword[i + 1] != 1 && _lstPassword[i + 1] != 2 && _lstPassword[i + 1] != 4)
                            flag = false;
                        break;
                    default:
                        break;
